...
Every role id starts with one of four prefixes Sourcer sets from that run's configuration — `X_P-`, `X_N-`, `LS_P-`, or `LS_N-` (see `CLAUDE.md`). You don't do anything differently for these — use the role id exactly as given, so your critique file naturally carries the prefix too (e.g. `output/LS_P-genentech-principal-pm-genai-agentic-ai/critique-v1.md`). This id prefix is unrelated to the `_LS` file-naming tag used for the `cv_LS` variant — the two are separate conventions.
